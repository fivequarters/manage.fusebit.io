...
const useRunner = () => {
    ...
    const getInstance = async () => {
        const { data: integration } = queryClient.getQueryData<ApiResponse<Integration>>([ACCOUNT_INTEGRATIONS_GET_ONE, { ...baseParams, id }]) || {}

        console.log('integration', integration)

        const { data: { items } } = await getAllInstances<Install>(axios, {
            ...baseParams,
            id,
        }, {
