...
const Delete = React.forwardRef(({open, onClose, handleDelete}: Props, ref) => {
    ...
    return (
        <SC.Card open={open}>
            ...
            <SC.ButtonsWrapper>
                <Button onClick={() => onClose()} style={{width: "77px", marginRight: "16px"}} size="medium" variant="outlined" color="primary">Cancel</Button>
                <Button onClick={() => handleDelete()} style={{width: "77px"}} size="medium" variant="contained" color="primary">Delete</Button>
            </SC.ButtonsWrapper>
        </SC.Card>
    )
})
